...
export default function ReportPreview({ data }: Props) {
  ...
  async function handleDownload() {
    setDownloading(true);
    try {
      const { pdf, Document, Page, View, Text, StyleSheet, Svg, Rect, Line, Path, G } =
      ...
      const ReportDocument = (
        <Document>
          <Page size={paperSize.toUpperCase() as "A4"} style={styles.page}>
            {/* Header — full COBACK logo as SVG */}
              ...
              <View style={styles.legendRow}>
                {scopes.map((scope) => (
                  <View key={scope.name} style={styles.legendItem}>
                    <View style={[styles.legendDot, { backgroundColor: scope.fill }]} />
      ...
      <CardContent className="space-y-4">
        <div className="flex items-end gap-4">
          <div className="space-y-2">
            ...
            <Select value={paperSize} onValueChange={(v) => setPaperSize(v as PaperValue)}>
        ...
        {/* On-screen preview */}
        <div className="max-h-[70vh] overflow-y-auto rounded-lg border border-border/50">
          <div
            className="mx-auto bg-white p-8 shadow-lg"
            style={{
              color: "#000",
              width: 595,
              minHeight: Math.round(
                595 / (PAPER_SIZES.find((s) => s.value === paperSize)?.ratio ?? 0.707)
              ),
            }}
          >
            ...
            <EmissionsChart data={data} forReport />
